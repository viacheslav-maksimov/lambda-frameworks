...
variable "project" { default = "spring-native-user-score" }

# Lambda function bucket
variable "lambda_bucket" { default = "test.lambda-package" }

# Lambda function bucket path
variable "lambda_bucket_path" { default = "spring-native-user-score" }
...
# to provision the runtime for the function language and which version
# supported by AWS.
...
variable "handler" { default = "org.springframework.cloud.function.adapter.aws.FunctionInvoker::handleRequest" }
